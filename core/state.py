...
class Agent:
    ...
    # **작년 소득.** 「나이를 먹으면 수입이 는다」 는 SYS 가 말하지만, 자기가 얼마나
    # 늘었는지는 작년 오프닝이 문맥에 남아 있어야 알 수 있다 — `inh30` 에서 프롬프트
    # 천장이 9,670 토큰이라 초기 해는 밀려났다. 새 정보가 아니라 이미 준 정보를
    # 잃지 않게 하는 것이다. 태어난 해는 0 이므로 적지 않는다.
    ...
    # **사람마다 다르다** (8/22). 태어날 때 뽑고 평생 안 바뀐다. 둘은 **독립**이다 —
    # 「고소득·저처리」 와 「저소득·고처리」 가 같이 생기고, 그 둘이 서로를 필요로 한다.
    # 그것이 `give` 와 `speak` 가 필수가 되는 지점이다.
    #
    # **남의 값은 보이지 않는다.** 보이게 하면 소통이 필요 없어진다 — 물어봐야 안다.
    income_mult: float = 1.0     # 소득 배수
    invest_mult: float = 1.0     # 한 번에 옮기는 액수의 배수 (학습에는 안 걸린다)
