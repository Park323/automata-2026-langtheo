...
FACTS: dict[str, tuple[Vis, str]] = {
    ...
    # **누가 우리 시설을 얼마나 움직였나** (8/26 · Eddie). 그 나라 사람 전원이 안다 —
    # 외국인이 손을 댄 것도 여기서 드러난다. 그러나 **의도는 안 드러난다**: 투자와
    # 파괴가 같은 통지를 쓰고, 역화가 있으므로 부호도 의도를 말하지 않는다
    # (투자가 음수일 수 있고 파괴가 양수일 수 있다).
    "impact":            (Vis.PUBLIC, "누가 우리 시설의 진척을 얼마 움직였나 (의도는 아니다)"),
