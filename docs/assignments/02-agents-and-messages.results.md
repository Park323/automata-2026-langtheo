...
아래는 **심층 리뷰 수정을 모두 반영한 코드**로 재실행한 결과 (에러 0 완주).

```
소요 시간   64.7s  (21.6s/턴)
에이전트    66콜   (in 128,662 / out 8,844 토큰)
번역        17콜   (in 1,077 / out 580 토큰)
이번 run 비용  ~$0.015   (에이전트 in$0.10·out$0.20/1M, 번역 in$0.094·out$0.25/1M)
생존 판정   intercept_failed  (3턴으론 요격기 미완성 — 정상)
```

**에이전트가 실제로 한 것 — 협력과 배신이 동시에 창발했다:**
- **국제 소통 20건.** A→B·C, B→A·C, C→A·B 등 서로 협력 의향을 교환. reasoning 에 *"함께 대응 방안 논의"*,
  *"협력 의향"*, *"공동 방어"* 가 반복됨.
- **🔥 배신(각자도생)도 창발.** C3 가 **자국에 벙커를 100 투자** — 다른 나라들이 요격기 협력을 추구하는 와중에
  자기만 살겠다는 선택. 설계가 노린 공공재 딜레마(협력 vs 배신)가 실제로 갈렸다.
- **언어 장벽이 작동.** `original`(원문 직통) 메시지가 여러 건 **전달 실패**(수신자가 못 읽음). 발신자는 다음 턴에
  **실패 통지**를 받아 되물을 근거를 얻음(리뷰에서 이 렌더 버그를 고침). 나머지는 `ai`(번역) 경로.
- **언어 학습 시도** + **자원 희소성**(예산 부족으로 speak 48 못 보냄) + **`propose_vote`·`invest`·`speak`** 전부 실제 호출.
- **에러 0.** malformed tool_call·HTTP 400·인코딩 수정이 모두 유효 — 3턴 전 구간 크래시 없이 완주.

> **본실험 비용 추정:** 3턴 66콜 → 50턴 ≈ 1,100콜, 1런 ≈ $0.25. 노브 4조건 × 시드 5 ≈ 20런 ≈ **$5~6**.
