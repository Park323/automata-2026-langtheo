...
| `interceptor: 8200` | ★C |
| `bunker_scale: 1000` | 벙커↓ |
| `success_prob: 0.5` (임계는 그대로) | ★C — 임계가 진척 단위임을 확인 |
| `comm_intl_ai: [4, 12]` (learner 5) | 노브 |

**마지막 줄이 중요합니다.** `success_prob`을 바꾸면 임계값을 재계산해야 하는데,
그걸 잊으면 세계가 조용히 망가집니다. 그래서 검사가 잡아야 합니다.
...
| 2 | 런당 사망 수 | **46 ~ 55** (2,000회 시뮬 평균 50.5, 표준편차 1.5) |
| 3 | 수명 분포 | 사망 나이 평균 **8.2 ~ 8.4**, 최대 관측 **12 이하** |
...
> **2번이 왜 46~55인가** — 기대수명 8.28턴이니 슬롯당 `50/8.28 ≈ 6`회 죽고
> 9슬롯이면 약 54회인데, 마지막 생애는 끝까지 살아 죽지 않으므로 조금 줄어듭니다.
> `tools/balance/sweep.py`의 출력은 약 40회로 더 적은데, 그건 `wellness` 투자가
> `λ`를 늘려 수명이 길어진 정책이 섞여 있기 때문입니다. **더미 정책은
> `wellness`에 투자하지 않으므로 46~55가 맞습니다.**
