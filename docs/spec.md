...
얼마를 넣으면 얼마가 오르는지를 모르게 두세요. 그래야 그것이 소통 소재가 되고,
경험으로 추정해야 하며, **유언으로 전달될 수 있습니다.**
*"우리가 30을 넣었는데 별로 안 올랐다"* 가 세대를 넘어 전해지는지가 관측 대상입니다.
...
> **7장의 `assert thresholds.interceptor > 3 × 마지막_세대_국가_최대자원`이 필수입니다.**
...
지출
  행복 구매        1당 행복 +1
  국가 투자        1당 행복 +0.05, 다음 세대부터 자국 생산 배수 상승
  시설 투자        1당 행복 +0.05, 자국 시설 진척에 기여
...
```
(a) pivot 경유 자체
(b) 에이전트가 요구한 압축 (translate_instruction)
(c) 언어 간 문법 구조 차이
(d) 길이 제약 하에서 발신자가 스스로 뺀 것
```

국내와 국제·AI의 차이가 곧 **번역 고유의 손실**입니다.
...
```
happiness += 투자액 × warm_glow_rate      # 즉시, 확정
```
...
```
① 세 투자처에 동일한 warm_glow_rate 를 적용할 것
   차등을 두면 "무엇이 더 고귀한가"를 우리가 정하는 셈이 되어
   결과가 설정의 귀결이 됩니다 (원칙 2 위반)

② warm_glow_rate < 1 / happiness_unit
...
warm_glow:
  rate: ?                  # 공공재 투자액 × rate 만큼 즉시 행복
                           # national / bunker / interceptor 에 동일 적용
...
# multiplier 는 국가 투자의 결과이므로, 상한 검증에는 "전 세대가 전액을
# 국가 투자에 쏟았을 때"의 최대 multiplier 를 써야 한다.
def 국가_최대자원(g):
    return income.work_income * 최대_multiplier(g) * agents_per_country * lifespan
...
assert warm_glow.rate < 1 / costs.happiness_unit
...
같은 내용의 문장 10개를 ja / zh / fr 로 각각 작성
  ...
  → 동일 글자 수로 두면 fr 이 구조적으로 불리해짐 (5.1.1)
```
...
```
thresholds.interceptor / thresholds.bunker / income.work_income / growth_coef
success_prob / costs.learn_new / world.lifespan / world.generations
```
