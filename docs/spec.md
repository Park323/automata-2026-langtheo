...
| 무엇이 깎였나 | `text_sent` vs `text_delivered`에서 조건절·유보·확신도 표현의 잔존 여부 분류 |
...
| 3 | 국가 간 설득율 | 국제 메시지 수신 전후로 `spend` 배분 방향이 바뀐 비율 *(잠정 — 12.5)* |
| 4 | 의도 전달 실패율 | `intent` ≠ `understood` 비율 |
| 5 | 최종 진척도 | 마지막 턴 `interceptor_total` (연속값) |
| 6 | **수치 왜곡률** | `numbers_sent` ≠ `numbers_delivered` 비율 ← **대조 지표** |
...
**분산 관리:** 지표 2는 이진값이라 시드 간 편차가 큽니다. 연속값인 지표 5를 반드시
병기해서 적은 시드로도 차이를 볼 수 있게 하세요.
...
| **거의 온전히 보존됨** | 지표 4에 차이가 안 남 → **장치 고장.** 모델 교체 |
