...
|---|---|
| 에이전트 | 전원 사망, 신규 9명 생성 |
| `budget` / `AP` | `income.initial_budget` 으로 초기화 / AP 는 매 턴 리셋 |
...
세대 g 시작 시 (직전 세대 종료 시점의 자본으로 계산)
  ...
  per_turn_g      = income.per_turn × multiplier_g
...
```
1. 턴 시작 시  budget += income.per_turn_g  (이월),  ap = turn.action_points  (리셋)
...
**예산은 이월되지만 AP는 이월되지 않습니다.** 세대 전환 시 초기화되므로(3.2) 최대 축적량은
`lifespan × income.per_turn_g`로 자연히 제한됩니다. 별도 상한 장치가 필요 없습니다.
...
```
예산 (budget)          매 턴 income.per_turn_g 만큼 자동 지급. 이월됨
행동력 (AP)            매 턴 turn.action_points 로 리셋. 이월 안 됨
```

`income.per_turn_g`는 자국의 생산 배수가 반영된 값입니다 (3.2).
...
| `learn` | `learn_new` 또는 `learn_inherit` | **`ap.learn` = 1.0 (생애의 한 턴을 통째로)** |
...
```
happiness += Σ(invest.amount where target ∈ {national, facility}) × warm_glow.rate
```
...
> `learn_new / (ai − learner)` 이므로 `learner` 가 커지면 분모가 줄어 손익분기가 밀립니다.
>
...
warm_glow:
  rate: ?                  # invest(national|facility) 합계 × rate 만큼 즉시 행복
                           # 두 항목에 동일 적용
...
def build_caps(cfg):
    ...
    for g in range(cfg.world.generations):
        ...
        cap = cfg.income.per_turn * m * cfg.world.agents_per_country * cfg.world.lifespan
...
저축_필요턴  = costs.learn_new / income.per_turn
...
턴당 9 + 27 = **36회**입니다. 현실적으로는 자국 내 메시지와 투자가 섞이므로 훨씬 적습니다.

**AP가 런타임을 결정합니다.** 다만 `ap.speak` 를 올려 소통을 줄이는 것은
**관측할 것 자체를 줄이는 일**이므로 최후 수단입니다.
런타임을 깎아야 하면 **세대 수·수명을 먼저 보세요** (12.7의 절단 순서 참조).
...
`income.per_turn`이 너무 낮으면 아무것도 못 하고, 너무 높으면 희소성이 사라집니다. `growth_coef`가 높으면 전 세대가 국가 투자만 하다 죽고, 낮으면
...
| **1** | `growth_coef = 0` (국가 투자·성장) | 세대 간 공공재 축, 후손 초기 행복 | 계승 비가역성, 유언, 3층 딜레마의 나머지 |
| **2** | 원문 직통 도박 (`route`를 시스템이 자동 결정) | "알려야 쓸모있다"는 재귀 | 학습의 검출 가치, 원문 병기 |
| **3** | 세대 2개로 | 구전 감쇠 | 계승 비가역성 |
