...
for p in sorted(pathlib.Path("runs").iterdir()):
    ...
    mt = [r for r in [json.loads(l) for l in (p/"metrics.jsonl").read_text(encoding="utf-8").splitlines() if l.strip()]
    ...
    sd = int(re.search(r"^seed: (\d+)", (p/"config_snapshot.yaml").read_text(encoding="utf-8"), re.M).group(1))
